/**
 * Raw data for countries, formatted as a CSV string.
 * 
 * Each line represents a country, with the following columns:
 * 
 * 1. #ISO: ISO 3166-1 alpha-2 code
 * 2. ISO3: ISO 3166-1 alpha-3 code
 * 3. ISO-Numeric: ISO 3166-1 numeric code
 * 4. fips: Federal Information Processing Standard (FIPS) code
 * 5. Country: Country name
 * 6. Capital: Capital city
 * 7. Area (in sq km): Country area in square kilometers
 * 8. Population: Country population
 * 9. Continent: Continent where the country is located
 * 10. tld: Top-level domain (TLD) for the country
 * 11. CurrencyCode: Currency code
 * 12. CurrencyName: Currency name
 * 13. Phone: Country phone code
 * 14. Postal Code Format: Format for postal codes
 * 15. Postal Code Regex: Regular expression for postal codes
 * 16. Languages: Languages spoken in the country
 * 17. geonameid: GeoName ID
 * 18. neighbours: Neighboring countries
 * 19. EquivalentFipsCode: Equivalent FIPS code
 */
export const rawData = `#ISO	ISO3	ISO-Numeric	fips	Country	Capital	Area(in sq km)	Population	Continent	tld	CurrencyCode	CurrencyName	Phone	Postal Code Format	Postal Code Regex	Languages	geonameid	neighbours	EquivalentFipsCode
